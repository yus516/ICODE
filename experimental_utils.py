    ...
    for i in range(len(lambdas)):
        ...
        for j in range(len(gammas)):
            ...
            for l in range(n_datasets):
                ...
                # exp_name = 'ramp'
                print("Saving data in ", exp_name)
                if not os.path.exists('tmp/' + exp_name):
                    os.makedirs('tmp/' + exp_name)
                np.save('tmp/' + exp_name + '/cyberattck9_' + exp_name + '_9_1_agc_a_hat_l_1-68_variable-3000' + str(l) + '.npy', a_hat_l)
                np.save('tmp/' + exp_name + '/cyberattck9_'  + exp_name + '_9_1_agc_a_hat_l_1-68_variable-3000_' + str(l) + '.npy', a_hat_l_)
                np.save('tmp/' + exp_name + '/cyberattck9_'  + exp_name +  '_9_1_agc_coeffs_full_l_1-68_variable-3000'  + str(l) +   '.npy', coeffs_full_l)
                # np.save('tmp/no_norm_relu_a_hat_l_signed.npy', a_hat_l_signed)
                ...
                # Avoid None type result. 
                acc_l, bal_acc_l, prec_l, rec_l, auroc_l, auprc_l = [
                    0 if v is None else v for v in [acc_l, bal_acc_l, prec_l, rec_l, auroc_l, auprc_l]
                ]
